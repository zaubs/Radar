    ...
    with open(path, 'r') as data:
        ...
        for row in data:
            ...
            # defining variables
                ...
                # turning radiant into heliocentric coordinates
                # v_cel      = getvec(float(alpha_g), float(delta_g))
                # v_hel      = cel2hel(v_cel, float(sl))
                ...
                if method == 'raw':
                    ...
                    parent_dict[date+time] = {"date": date, "time": time, "Number of Stations" : num_stations, "R0" : R0, "Theta" : theta, "Phi" : phi, "Time of flight velocity": vel_TimeofFlight, "Pre-t0 velocity": vel_PTN0, "Geocentric velocity" : vel_geo,
                                          ...
                                          "Cel2Hel Longitude" : corrected_l_comp, "Cel2Hel Latitude" : b_comp}
...
def voxel_map(lmda, beta, vels, year, name=None, map_mode='shower', threshold=0, bounds=None):
    '''
...
def shower_parser(year, folder, file, file_slon, slon_peak, radiants, radiant_drifts, name, method, slon_status='active'):
    '''
        ...
        with open(path, 'r') as shower_data, open(outer_path, 'w') as outer_file:
            for line in shower_data:
                ...
                if params == []:
                    ...
                    # if abs(shower_lmda - meteor_lmda) <= 5 and abs(shower_beta - meteor_beta) <= 5:
                    date_time.append(f'{date} {time}')
                    shower_lons.append(lmda)
                    shower_lats.append(beta)
                    shower_vels.append(velg)
...
def voxel_subtract(year, shower_counts, background_counts, name, method, slon_status='active'):
    '''
    ...
    for i, matrix in enumerate(background_counts): # should be going over each matrix creating the 3d array
    ...
    return shower_counts_copy


def coord_sigma(shower_lmda, shower_beta, shower_vels, slons, year):
    ...
    # new_lmda = shower_lmda[mask]
    # new_beta  = shower_beta[mask]
    # new_vels  = shower_vels[mask]

    for l, b, v in zip(shower_lmda, shower_beta, shower_vels):
    ...
    return new_lmda, new_beta, new_vels
...
if source_isolate:
    ...
    # echo_plot(c2h_lons, c2h_lats, plot_vels, year, method, map_mode=map_mode) # heliocentric coordinates not 1-1 with ecliptic coordinates
...
if shower_isolate:
    ...
    for shower, rads in shower_rads.items():
        ...
        lon, lat = getangle(hel)

        scaled_lon = scale(lon) # bring to a 0-360 scale

        shower_hels[shower] = [float(scaled_lon), float(lat)]
    
    print(shower_hels)
    
    shower_rad_drifts = {'ARI' : [0.86, 0.18], 'DSX' : [-1.00, 0.56], 'ETA' : [0.70, 0.33],
                         'GEM' : [1.12, -0.17], 'QUA' : [0.78, -0.38], 'SDA' : [0.78, 0.30]} # 'KEY' : [alpha drift value, delta drift value] both in degrees
    ...
    for shower_file in shower_folder:
        ...
        if file_slon in full_slons:
            ...
            if file_slon in input_slons:
                ...
                shower_lmda, shower_beta, shower_vel, corr_shower_radiants, shower_meteor_count, shower_days = shower_parser(year, shower_folder_name, shower_file, file_slon, slon_peak, shower_rads, shower_rad_drifts, shower_name, method, slon_status='active')
                ...
                # keeping track of number of meteors deemed close enough to the shower for a given solar longitude
            ...
            # using these to construct average sporadic number density matrices to subtract from the shower's number density matrix (heat map)
                ...
                # keeping track of number of meteors close enough to the source in background days of the shower
    ...
    print('Number of shower meteors: ', shower_meteors)
    ...
    h_background = echo_plot(scaled_outer_lons, outer_lats, outer_vels, year, method, shower=shower_name, mode='background', map_mode=map_mode, bounds=[shower_lon_bounds, shower_lat_bounds])
    # print('mean:', np.mean(h_background))

    print('Number of background meteors: ', background_meteors)
    ...
    masked_lons, masked_lats, masked_vels = echo_3d_plot(scaled_outer_lons, outer_lats, outer_vels, year, shower=shower_name, mode='shower', bounds=[shower_lon_bounds, shower_lat_bounds, shower_vel_bounds])
    ...
    H_shower, edges_shower, shower_lons, shower_lats, shower_vels = voxel_map(scaled_active_lons, active_lats, active_vels, year, name=shower_name)
    H_background, edges_background, background_lons, background_lats, background_vels = voxel_map(scaled_outer_lons, outer_lats, outer_vels, year, name=shower_name, bounds=[shower_lon_bounds, shower_lat_bounds, shower_vel_bounds])
    ...
    diff_lons2 = diff_lons[shower_mask]
    diff_lats2 = diff_lats[shower_mask]
    diff_vels2 = diff_vels[shower_mask]
    diff_days2 = diff_days[shower_mask]

    print(len(diff_lons2), len(diff_lats2), len(diff_vels2), len(diff_days2))

    # for l, b, v in zip (shower_lons, shower_lats, shower_vels):
    ...
    diff_count = len(diff_lons) - len(diff_lons2)
    
    print(f'\nFrom voxel subtraction, {diff_count} meteors were removed from the shower for overlapping with background sporadics.\n')
    print(f'Currently, there are {len(diff_lons2)} meteors contained within the shower\'s radiant\n')
    ...
    # after 3 sigma clip
    H_prime = voxel_subtract(year, H_diff, new_Background, shower_name, method) # will need to get lats/lons from this step and feed to coord_sigma
    # print(H_prime)

    # voxel_map2(H_prime, edges_shower, scaled_active_lons, active_lats)
    voxel_map2(H_prime, edges_shower, diff_lons, diff_lats, diff_vels, threshold=0)
    ...
    final_lons, final_lats, final_vels = coord_sigma(diff_lons, diff_lats, diff_vels, full_slons, year)
    ...
    print('\nArray lengths', len(active_lons), len(diff_lons), len(final_lons)) # currently not changing in the coord_sigma test -> checking within 3 sigma which might be too high
    ...
    voxel_map2(H_final, edges_final, final_lons, final_lats, final_vels, threshold=5)
